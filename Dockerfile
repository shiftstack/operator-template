...
WORKDIR /go/src/github.com/shiftstack/operator-template
COPY . .
RUN make

FROM registry.svc.ci.openshift.org/openshift/origin-v4.0:base
COPY --from=builder /go/src/github.com/shiftstack/operator-template/openstack-cinder-csi-driver-operator /usr/bin/
